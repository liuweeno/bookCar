...
router.get('/CourseStudentList', (req, res) => {
  ...
  for (let i = 0; i < 54; i++) {
    let student = {
      ...
      name:
        courseStudent.lastName[random_int(0, 11)] +
        courseStudent.name1[random_int(0, 9)] +
        (random_int(0, 9) > 1 ? courseStudent.name2[random_int(0, 9)] : ''),
  ...
  if (resdata.timetableList.length == 0) {
    ...
    for (let item of timetableList) {
      ...
      for (let i = 0; i < weeklen; i++) {
        ...
        if (slotLen > slotLimit[dayi][1] - slotLimit[dayi][0] + 1)
          slotLen = slotLimit[dayi][1] - slotLimit[dayi][0] + 1;
