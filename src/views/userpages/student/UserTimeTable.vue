  ...
  <div class="appmain">
    <div class="appcart selcart">
      <div class="selmode-wrap">
        <span class="selmode" :class="{ active: params.selMode === 'week' }" @click="changeMode('week')"
          >按周次查询</span
        >
        <span class="selmode" :class="{ active: params.selMode === 'course' }" @click="changeMode('course')"
          >按课程查询</span
        >
      </div>
      ...
      <SelectBlock
        atr="周次"
        describe="请选择"
        v-model="params.selParam"
        v-show="params.selMode === 'week'"
        :change="updata"
      >
    ...
    <transition name="cart-fade" mode="out-in">
      ...
      <div class="appcart" v-if="params.selMode === 'week'" key="week">
        <table class="apptable week-table">
          ...
          <tr v-for="i in 13" class="timeslot-row" :key="i">
            ...
            <td
              v-for="j in 7"
              v-show="spanRule[j - 1][i - 1]"
              :rowspan="spanRule[j - 1][i - 1]"
              :class="{ 'course-exist': data.weekItemSite[j.toString() + i] }"
              :key="j"
            >
              <div class="course-wrap" v-if="data.weekItemSite[j.toString() + i]">
                <p class="course-name" @click="intoCourseList(data.weekItemSite[j.toString() + i].course)">
                  {{ data.weekItemSite[j.toString() + i].course }}
                </p>
      ...
      <div class="appcart" v-else key="course">
        <table class="apptable course-table">
          ...
          <tr v-show="!data.courseList.length">
