  ...
  <div class="appmain">
    ...
    <div class="appcart">
      <table class="classroom-table">
        ...
        <tr v-show="classRoomQueryList.list.length == 0">
        ...
        <tr v-for="item in classRoomQueryList.list" :key="item.Cid">
          ...
          <td>
            <button
              class="reserve-bt"
              :class="{ banbt: item.arrangeList[selParams.timeslot * 1] !== '空闲' }"
              @click="reserveRoom(item)"
            >
              预 定
            </button>
          </td>
      ...
      <div class="pagination-wrap" v-if="classRoomQueryList.totalPage > 0">
        <span>
          <Pagination
            :current_page="classRoomQueryList.pageNo"
            :totel_page="classRoomQueryList.totalPage"
            :continues="5"
