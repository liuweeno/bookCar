  ...
  <div class="appmain">
    <div class="appcart selcart">
      <div class="selmode-wrap">
        ...
        <span class="selmode" :class="{ active: data.selMode === 'course' }" @click="changeMode('course')"
          >按课程查询</span
        >
      </div>
      ...
      <SelectBlock
        atr="周次"
        describe="请选择"
        v-model="data.modeParam"
        v-show="data.selMode == 'week'"
        :change="updata"
      >
    ...
    <transition name="cart-fade" mode="out-in">
      <div class="appcart" v-if="data.selMode == 'week'" key="week">
        <table class="apptable week-table">
          ...
          <tr v-for="i in 13" :key="i">
            ...
            <td
              v-for="j in 7"
              v-show="arList.spanRule[j - 1][i - 1]"
              :rowspan="arList.spanRule[j - 1][i - 1]"
              :class="{ 'course-exist': arList.List[j.toString() + i] }"
              :key="j"
            >
              <div class="course-wrap" v-if="arList.List[j.toString() + i]">
                ...
                <p class="course-timeslot">
                  {{ arList.List[j.toString() + i].startSlot + '-' + arList.List[j.toString() + i].endSlot + ' 节' }}
                </p>
              </div>
            </td>
          </tr>
      ...
      <div class="appcart" v-else key="course">
        <table class="apptable">
          ...
          <tr v-show="!arList.modeCourseData.arrange">
...
function updataSpanRule(courseList) {
  ...
  for (let item of courseList) {
    ...
    if (
      item.weekArrange[0] == 6 ||
      (item.weekArrange[0] == 7 && item.weekArrange[1] == 3) ||
      item.weekArrange[1] == 9
    ) {
