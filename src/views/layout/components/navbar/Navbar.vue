<template>
  <div class="nav-wrap">
    ...
    <span class="menu-level">
      <a class="menulink" @click="router.push({ name: data.menuNav.mainMenu.name })">{{
        data.menuNav.mainMenu.meta.title
      }}</a>
      <a
        class="menulink sub"
        v-for="item in data.menuNav.subMenu"
        :key="item.meta.title"
        :class="{ last: !item.children }"
    ...
    <i class="iconfont icon-font-size right-icon font-set" @click="showFontDiv">
      <div
        ...
        tabindex="-1"
        @blur="hiddenFontDiv"
        @mouseleave="setIsOut(true)"
        @mouseenter="setIsOut(false)"
        ref="fontDiv"
      >
        <span>字体尺寸：</span>
